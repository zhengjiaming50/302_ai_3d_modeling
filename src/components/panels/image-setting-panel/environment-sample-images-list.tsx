...
const logger = createScopedLogger("EnvironmentSampleImagesList");

interface EnvironmentSampleImagesListProps {
  onSelected: (imageSrc: string, imagePrompt: string) => void;
}

export const EnvironmentSampleImagesList = memo(
  function EnvironmentSampleImagesList({
    onSelected,
  }: EnvironmentSampleImagesListProps) {
    const t = useTranslations("environment.sample_images");
    ...
    const handleSelected = (imageSrc: string, imagePrompt: string) => {
      ...
      logger.info("Environment image selected:", imageSrc, imagePrompt);
    };
    ...
    return (
      <div className="flex flex-col gap-2">
        ...
        <div className="flex w-full flex-row items-center justify-start gap-4 overflow-x-auto rounded-lg border border-border p-4">
          {GLOBAL.IMAGE_SAMPLE_LIST.ENVIRONMENT_IMAGE_LIST.map((item) => (
            <div
