...
import { getStablePoint3DModeling } from "@/services/modeling";
...
interface UseStablePoint3DAsyncProps {
...
export function useStablePoint3DAsync({
  ...
  const handleGenerateStablePoint3DModel = useCallback(async () => {
    ...
    const getModeling = async () => {
      ...
      return await getStablePoint3DModeling(imageFile);
    };
    toast.promise(getModeling(), {
      ...
      success: async (response) => {
        ...
        updateModelViewer({
          ...
          modelingModel: "StablePoint3D",
          key: createAt,
        });

        setIsGenerating(false);

        logger.info("StablePoint3D modeling:", uploadedFile.url);
      ...
      error: (error) => {
        ...
        setIsGenerating(false);
        logger.error("Error generating stable point 3d modeling", error);
